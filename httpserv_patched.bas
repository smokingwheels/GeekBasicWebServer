...
Rem tested to 500 Hz by smokingwheels
...
Const users = 2000 'total users allowed to connect at a time
Const vars = 2000
Const lines = 2000
    ...
    If userid(user) Then
        ...
        If indata$ <> "" Then
            ...
            If Left$(UCase$(indata$), 5) = "GET /" Then
                ...
                'Directly check the requested path instead of rebuilding and scanning files.dat.
                Let requestedfile$ = indata$
                Let requestedpath$ = "www" + pathsep$ + requestedfile$
                Let foundpage = 0
                Let ispng = 0

                'Block simple attempts to escape the www directory.
                If InStr(requestedfile$, "..") = 0 And InStr(requestedfile$, ":") = 0 Then
                    If _FileExists(requestedpath$) Then Let foundpage = -1
                End If

                If foundpage Then

                    If Right$(UCase$(requestedfile$), 4) = ".PNG" Then
                        Let ispng = -1
                        Open requestedpath$ For Binary As #1
                    Else
                        Open requestedpath$ For Input As #1
                    End If
                    ...
                    While Not EOF(1)
                        ...
                        If Right$(UCase$(requestedfile$), 5) = ".HTML" Or Right$(UCase$(requestedfile$), 4) = ".HTM" Then
                            Line Input #1, indata$
                            Let indata$ = LTrim$(RTrim$(indata$)) + crlf$
                            Let page$ = page$ + indata$

                        ElseIf Right$(UCase$(requestedfile$), 5) = ".GBWS" Then
                            Line Input #1, indata$
                            Let indata$ = LTrim$(RTrim$(indata$))
...
Else
    ...
    Open "www" + pathsep$ + p$ For Output As #2
...
Else
    ...
    Open "www" + pathsep$ + p$ For Input As #2
...
Else
    ...
    Open "www" + pathsep$ + p$ For Append As #2
